...
const getJobRankScore = (job) => {
  ...
  if (!isDone && isPriority) {
    return 10000000000 + createdAt;
  }
  if (!isDone && !isPriority) {
    return 20000000000 + createdAt;
  }
...
const Card = ({ children, onClick, className = "" }) => (
  <div
    onClick={onClick}
    className={`bg-white p-4 rounded-2xl shadow-sm border border-slate-100 active:bg-slate-50 transition-colors ${className}`}
  >
    {children}
  </div>
);
...
export default function ElectroManager() {
  ...
  useEffect(() => {
    ...
    const unsubSites = onSnapshot(
      ...
      (snap) => setRawSites(snap.docs.map((d) => ({ id: d.id, ...d.data() })))
    );
    const unsubJobs = onSnapshot(
      collection(db, "artifacts", appId, "users", user.uid, "jobs"),
      (snap) => setRawJobs(snap.docs.map((d) => ({ id: d.id, ...d.data() })))
    );
    return () => {
      unsubClients();
      unsubSites();
  ...
  const validSites = useMemo(() => {
    return rawSites.filter((site) =>
      rawClients.some((client) => client.id === site.clientId)
    );
  }, [rawSites, rawClients]);

  const validJobs = useMemo(() => {
    return rawJobs.filter((job) =>
      validSites.some((site) => site.id === job.siteId)
    );
  }, [rawJobs, validSites]);

  // Helper function to count active jobs
  const getActiveJobsCount = (siteId) => {
    return validJobs.filter(
      (j) =>
        j.siteId === siteId && j.status !== "done" && j.status !== "cancelled"
    ).length;
  };

  // Helper function to count active sites for a client (sites with at least 1 active job)
    ...
    return validSites
      .map((site) => {
        ...
        if (siteJobs.length === 0)
          return { ...site, rankScore: 9999999999999, activeJobs: 0 };
  ...
  const filteredClients = useMemo(
    () =>
      clientsWithRank.filter((c) =>
        c.name?.toLowerCase().includes(searchTerm.toLowerCase())
      ),
    [clientsWithRank, searchTerm]
  );

  const displayedSites = useMemo(() => {
    let list = sitesWithRank;
    if (selectedClient)
      list = list.filter((s) => s.clientId === selectedClient.id);
    ...
    if (searchTerm) {
      ...
      list = list.filter(
        (j) =>
          ...
          j.offerRef?.toLowerCase().includes(lower) ||
          j.technicianNotes?.toLowerCase().includes(lower)
      );
    }
  ...
  const priorityJobsList = validJobs
    .filter(
      (j) => j.isPriority && j.status !== "done" && j.status !== "cancelled"
    )
  ...
  const handleLogin = () => {
    ...
    if (pinInput === storedPin) {
      ...
      alert("PIN Errato. Se è il primo accesso prova 1234");
    }
  };
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...formData,
      createdAt: modalMode === "add" ? serverTimestamp() : undefined,
      status: formData.status || "todo",
    };
    if (modalMode === "edit") delete payload.createdAt;
    if (view === "sites" && modalMode === "add") {
      payload.clientId = selectedClient.id;
      payload.clientName = selectedClient.name;
    }
    if (view === "jobs" && modalMode === "add") {
      payload.siteId = selectedSite.id;
      payload.clientName = selectedClient
        ? selectedClient.name
        : formData.clientName;
      payload.siteName = selectedSite ? selectedSite.name : formData.siteName;
    }
    ...
    try {
      await deleteDoc(
        doc(
          ...
          "users",
          user.uid,
          deleteConfirm.collection,
          deleteConfirm.id
        )
      );

      if (deleteConfirm.collection === "clients") {
        const sitesToDelete = rawSites.filter(
          (s) => s.clientId === deleteConfirm.id
        );
        for (const s of sitesToDelete) {
          await deleteDoc(
            doc(db, "artifacts", appId, "users", user.uid, "sites", s.id)
          );
          const jobsToDelete = rawJobs.filter((j) => j.siteId === s.id);
          for (const j of jobsToDelete)
            await deleteDoc(
              doc(db, "artifacts", appId, "users", user.uid, "jobs", j.id)
            );
        }
      } else if (deleteConfirm.collection === "sites") {
        const jobsToDelete = rawJobs.filter(
          (j) => j.siteId === deleteConfirm.id
        );
        for (const j of jobsToDelete)
          await deleteDoc(
            doc(db, "artifacts", appId, "users", user.uid, "jobs", j.id)
  ...
  const handleUpdateStatus = async (jobId, newStatus) => {
    if (newStatus === "done") {
      ...
      if (!job.endDate) {
        alert("⚠️ Data Fine Lavori obbligatoria per completare.");
        return;
      }
    }
    await updateDoc(
      doc(db, "artifacts", appId, "users", user.uid, "jobs", jobId),
      { status: newStatus }
    );
  };
  ...
  if (!isAuthenticated) {
    return (
      <div className="h-screen bg-slate-900 flex flex-col items-center justify-center p-6 text-white animate-in fade-in">
        <div className="bg-blue-600 p-4 rounded-full mb-6 shadow-lg shadow-blue-500/50">
          <Lock size={48} />
        </div>
        <h1 className="text-3xl font-bold mb-2">ElectroManager</h1>
        ...
        <div className="w-full max-w-xs space-y-4">
          <input
            ...
            pattern="[0-9]*"
            inputMode="numeric"
            className="w-full text-center text-2xl tracking-widest bg-slate-800 border border-slate-700 rounded-xl py-4 focus:ring-2 focus:ring-blue-500 outline-none text-white"
            placeholder="P I N"
            maxLength={4}
            value={pinInput}
    ...
    <div className="space-y-6 pb-24 animate-in fade-in">
      <div className="bg-slate-900 -m-4 mb-2 p-6 pb-8 rounded-b-3xl text-white">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="bg-slate-800 p-2 rounded-full text-slate-400 hover:text-blue-400"
            >
              <Settings size={20} />
            </button>
            <button
              onClick={handleLogout}
              className="bg-slate-800 p-2 rounded-full text-slate-400 hover:text-red-400"
            >
              <LogOut size={20} />
            </button>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-4">
          <div
            ...
            className="bg-slate-800 p-4 rounded-2xl border border-slate-700 active:bg-slate-700 active:scale-95 transition-all cursor-pointer"
          >
            <div className="text-slate-400 text-xs font-medium mb-1">
              Cantieri
            </div>
            <div className="text-2xl font-bold text-white flex items-center gap-2">
              {totalSitesCount} <MapPin size={16} className="text-orange-500" />
            </div>
          </div>
          <div
      ...
      <div className="px-1 grid grid-cols-2 gap-3">
        <Card
          ...
          className="bg-gradient-to-br from-blue-600 to-blue-700 border-none text-white shadow-lg"
        >
          <div className="flex flex-col items-center py-2">
            <User size={24} className="mb-2 text-blue-200" />
            <span className="font-bold">Clienti</span>
          </div>
        </Card>
        <Card
          onClick={() => setView("reports")}
          className="bg-gradient-to-br from-purple-600 to-purple-700 border-none text-white shadow-lg"
        >
          <div className="flex flex-col items-center py-2">
            <FileBarChart size={24} className="mb-2 text-purple-200" />
            <span className="font-bold">Report</span>
          </div>
        </Card>
      </div>
      <div className="px-1">
        <h3 className="font-bold text-slate-800 mb-3 flex items-center gap-2 mt-4">
        ...
        {priorityJobsList.length === 0 ? (
          <div className="text-center py-6 bg-white rounded-2xl border border-dashed border-slate-200">
            ...
            />
            <p className="text-slate-400 text-sm">Nessuna emergenza.</p>
          </div>
        ) : (
        ...
        <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 p-3 rounded-xl border border-slate-300 focus:border-blue-500 outline-none text-center tracking-widest text-lg"
              placeholder="####"
            />
            <Button onClick={handleChangePin}>Salva</Button>
          </div>
          <p className="text-xs text-slate-400 mt-2">
            Il PIN viene salvato solo su questo dispositivo.
          </p>
        </div>
      </div>
    </div>
  ...
  const renderReportsView = () => {
    ...
    if (reportType === "materials")
      filteredReportJobs = validJobs.filter(
        (j) => j.status === "order_material"
      );
    else if (reportType === "quotes")
      filteredReportJobs = validJobs.filter((j) => j.status === "quote_needed");
      ...
      <div className="space-y-4 pb-24 animate-in slide-in-from-right duration-200 bg-white min-h-screen">
        <div className="bg-slate-900 sticky top-0 z-20 p-4 text-white flex items-center justify-between shadow-md print:hidden">
          ...
          <button
            ...
            className="p-2 bg-slate-800 rounded-full hover:text-blue-400"
          >
            <Printer size={20} />
          </button>
        </div>
        <div className="px-4 print:p-0">
          <div className="flex gap-2 mb-6 overflow-x-auto pb-2 print:hidden">
            ...
            >
              Materiali (
              {validJobs.filter((j) => j.status === "order_material").length})
            </button>
            <button
            ...
            >
              Preventivi (
              {validJobs.filter((j) => j.status === "quote_needed").length})
            </button>
            <button
          ...
          <div className="space-y-4 print:space-y-2">
            <div className="hidden print:block mb-4 border-b pb-2">
              ...
              <p className="text-sm font-bold uppercase mt-2">
                {reportType === "materials"
                  ? "Lista Materiali da Ordinare"
                  : reportType === "quotes"
                  ? "Lista Preventivi da Fare"
                  : "Lista Lavori Attivi"}
              </p>
            </div>
            {filteredReportJobs.length === 0 ? (
              <div className="text-center py-10 text-slate-400">
                Nessun dato per questo report.
              </div>
            ) : (
              filteredReportJobs.map((job) => (
                <div
                  key={job.id}
                  className="bg-white border border-slate-200 rounded-xl p-4 print:border-black print:rounded-none print:border-0 print:border-b"
                >
                  <div className="flex justify-between items-start mb-2">
                    <div className="text-xs text-slate-500 font-bold uppercase tracking-wider mb-1 print:text-black">
                      {job.clientName || "Cliente"} &bull;{" "}
                      {job.siteName || "Cantiere"}
                    </div>
                    {job.isPriority && (
                      <span className="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded font-bold print:border print:border-black print:text-black">
                        PRIORITARIO
                      </span>
                    )}
                  </div>
                  <h3 className="font-bold text-lg text-slate-900 leading-tight mb-2 print:text-black">
                    {job.description}
                  </h3>
                  {job.technicianNotes && (
                    <div className="bg-slate-50 p-3 rounded-lg text-sm text-slate-700 print:bg-transparent print:p-0 print:text-xs print:italic">
                  ...
                  {job.offerRef && (
                    <div className="mt-2 text-xs font-mono bg-slate-100 inline-block px-2 py-1 rounded print:bg-transparent print:p-0">
                      RIF: {job.offerRef}
                    </div>
                  )}
                </div>
    ...
    <div className="space-y-4 pb-24 animate-in slide-in-from-right duration-200">
      ...
      <div className="px-4 relative">
        ...
        <input
          ...
          className="w-full bg-slate-100 border-none rounded-xl py-3 pl-10 pr-4 text-slate-700 focus:ring-2 focus:ring-blue-500 transition-all mb-4"
          value={searchTerm}
            ...
            <Card key={client.id} onClick={() => navigateTo("sites", client)}>
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    <div className="flex items-center gap-2">
                      ...
                      {client.activeSites > 0 && (
                        <span className="text-[10px] font-bold bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">
                          {client.activeSites} Attivi
                        </span>
                      )}
                    </div>
                    {client.rankScore < 20000000000 && (
                      <span className="flex items-center gap-1 text-[10px] text-red-600 font-bold uppercase mt-0.5">
                        <Flame size={10} fill="currentColor" /> Richiede
                        Attenzione
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={(e) =>
                      requestDelete("clients", client.id, client.name, e)
                    }
      ...
      <div className="fixed bottom-6 right-6">
        <button
          ...
          className="bg-blue-600 text-white p-4 rounded-full shadow-lg shadow-blue-300 hover:bg-blue-700 transition-transform hover:scale-105 active:scale-95"
        >
          <Plus size={24} />
        </button>
      </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="overflow-hidden">
            <h2 className="font-bold text-lg text-slate-800 leading-tight">
              {selectedClient ? selectedClient.name : "Tutti i Cantieri"}
            </h2>
      ...
      <div className="px-4 space-y-3">
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full bg-slate-100 border-none rounded-xl py-3 pl-10 pr-4 text-slate-700 focus:ring-2 focus:ring-blue-500 transition-all"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
        </div>
        {displayedSites.map((site) => (
          <Card
            key={site.id}
            onClick={() => navigateTo("jobs", selectedClient, site)}
          >
            <div className="flex justify-between items-start">
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  onClick={(e) => requestDelete("sites", site.id, site.name, e)}
                  className="p-2 text-slate-300 hover:text-red-500"
                >
                  <Trash2 size={18} />
                </button>
      ...
      {selectedClient && (
        <div className="fixed bottom-6 right-6">
          <button
            ...
            className="bg-orange-500 text-white p-4 rounded-full shadow-lg shadow-orange-200 hover:bg-orange-600 transition-transform hover:scale-105 active:scale-95"
          >
            <Plus size={24} />
          </button>
        </div>
    ...
    <div className="space-y-4 pb-24 animate-in slide-in-from-right duration-200">
      <div className="bg-white sticky top-0 z-10 p-4 border-b border-slate-100 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button
            onClick={() => {
              if (selectedSite) {
                navigateTo("sites", selectedClient);
              } else {
                navigateTo("sites");
              }
            }}
          ...
          <div className="overflow-hidden">
            <h2 className="font-bold text-lg text-slate-800 truncate">
              {selectedSite ? selectedSite.name : "Tutti i Lavori"}
            </h2>
            <p className="text-xs text-slate-500">
              {selectedSite ? "Lavori da eseguire" : "Ordinati per urgenza"}
            </p>
          </div>
        </div>
        <button
      ...
      <div className="px-4 space-y-3">
        ...
        {displayedJobs.map((job) => {
          const status =
            STATUS_TYPES.find((s) => s.id === job.status) ||
            STATUS_TYPES.find((s) => s.id === "todo");
          const type = JOB_TYPES.find((t) => t.id === job.type) || JOB_TYPES[3];
          return (
            <div
              key={job.id}
              className={`relative overflow-hidden rounded-2xl shadow-sm border p-4 transition-colors ${status.color}`}
            >
              <div className="flex flex-col gap-3">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-2 font-medium text-sm opacity-90">
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase cursor-pointer bg-white shadow-sm whitespace-nowrap text-slate-800">
                ...
                <div className="z-0">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {job.isPriority && (
                      <span className="flex items-center gap-1 text-xs bg-red-600 text-white px-2 py-0.5 rounded-full font-bold shadow-sm">
                        <Flame size={12} fill="currentColor" /> PRIORITARIO
                      </span>
                    )}
                    {job.offerRef && (
                      <span className="flex items-center gap-1 text-xs bg-white px-2 py-0.5 rounded-full font-bold text-slate-700 shadow-sm">
                        <Hash size={12} /> {job.offerRef}
                      </span>
                    )}
                  </div>
                  {!selectedSite && (
                    <p className="text-xs opacity-75 mt-2 font-medium">
                      ...
                      {job.siteName || "Cantiere sconosciuto"}
                    </p>
                  )}
                ...
                {job.technicianNotes && (
                  <div className="bg-white/50 p-3 rounded-lg text-sm border border-white/40 mt-1 shadow-sm">
                ...
                <div className="flex justify-end pt-3 border-t border-black/10 mt-1">
                  <button
                    ...
                    className="text-xs flex items-center gap-1 hover:opacity-100 opacity-70 mr-4 font-bold"
                  >
                    <Pencil size={14} /> Modifica
                  </button>
                  <button
                    onClick={(e) =>
                      requestDelete("jobs", job.id, job.description, e)
                    }
                    className="text-xs flex items-center gap-1 hover:opacity-100 opacity-70 font-bold text-red-800"
                  >
      ...
      {selectedSite && (
        <div className="fixed bottom-6 right-6">
          <button
            onClick={() => {
              setFormData({ status: "todo", type: "electric" });
              setModalMode("add");
              setIsModalOpen(true);
            }}
            className="bg-indigo-600 text-white p-4 rounded-full shadow-lg shadow-indigo-300 hover:bg-indigo-700 transition-transform hover:scale-105 active:scale-95"
          >
            <Plus size={24} />
          </button>
        </div>
      ...
      {view === "jobs" && (
        <>
          ...
          <div className="flex items-center gap-2 bg-red-50 p-3 rounded-xl border border-red-100">
            ...
            >
              <Flame size={16} fill="currentColor" /> Contrassegna come
              Prioritario
            </label>
          </div>
          <Input
            ...
            placeholder="Es. Installazione..."
          />
          <Input
            label="Rif. Offerta"
            value={formData.offerRef || ""}
            onChange={(e) =>
              setFormData({ ...formData, offerRef: e.target.value })
            }
          ...
          <textarea
            className="w-full p-3 border rounded-xl bg-slate-50 text-sm h-24"
            ...
            <div className="bg-white w-full max-w-md rounded-2xl p-6 shadow-2xl animate-in slide-in-from-bottom-10">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-xl">
                  {modalMode === "add" ? "Aggiungi Nuovo" : "Modifica"}
                </h3>
                <button
