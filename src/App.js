...
export default function ElectroManager() {
  ...
  useEffect(() => {
    if (!user) return;
    const unsubClients = onSnapshot(collection(db, 'artifacts', appId, 'users', user.uid, 'clients'), (snap) => setRawClients(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    const unsubSites = onSnapshot(collection(db, 'artifacts', appId, 'users', user.uid, 'sites'), (snap) => setRawSites(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    const unsubJobs = onSnapshot(collection(db, 'artifacts', appId, 'users', user.uid, 'jobs'), (snap) => setRawJobs(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
  ...
  const handleExportData = async () => {
    ...
    try {
      ...
      link.download = `ElectroManager_Backup_${new Date().toISOString().slice(0,10)}.json`;
      link.href = url;
      link.click();
